...
struct TeamHook: ParsableCommand {
    ...
    func run() throws {
        ...
        do {
            ...
            case .error:
                print("{}")
            case .ok, .paneList, .paneShow, .teamList, .teamInbox, .flowStatus, .flowContext, .flowRecommendation:
                print("{}")
            }
        } catch {
            print("{}")
...
struct TeamInbox: ParsableCommand {
    ...
    func run() throws {
        ...
        case .error(let msg):
            ...
            throw ExitCode(1)
        case .ok, .paneList, .paneShow, .teamList, .teamHookOutput, .flowStatus, .flowContext, .flowRecommendation:
...
private enum TeamOutput {
    static func printMembers(_ response: ResponseMessage) throws {
        ...
        case .error(let msg):
            CLIEnv.printError(msg)
            throw ExitCode(1)
        case .ok, .paneList, .paneShow, .teamHookOutput, .teamInbox, .flowStatus, .flowContext, .flowRecommendation:
