...
enum WorktreeResolver {
    ...
    /// Graftty. Throws `.notInsideWorktree` if either (a) PWD isn't inside
    /// any git repo or worktree, or (b) the resolved worktree isn't tracked
    /// by the running Graftty app.
    ///
    ...
    /// unknown name. Reads the same `state.json` `isTracked` does and
    /// delegates the matching to `WorktreeNameLookup` so a name
    /// accepted by `graftty team msg` is also accepted here.
