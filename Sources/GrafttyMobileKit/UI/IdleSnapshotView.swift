...
/// `.idle` so libghostty's display link can stop. Renders a supplied
/// snapshot if one exists, otherwise falls back to a black placeholder.
/// A full-bleed tap target invokes `onWake`.
