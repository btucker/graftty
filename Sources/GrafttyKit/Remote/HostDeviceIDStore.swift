...
// MARK: - HostDeviceIDStore

/// Persists the host's stable `RemoteDeviceID` so pairing payloads and
/// trusted-peer records identify the same Mac across launches.
///
/// The ID is stored in a JSON file (`host-device-id.json`) inside the
/// given directory, alongside a schema version, mirroring
/// `HostIdentityStore`'s persistence idiom. Safe to use from concurrent
...
public final class HostDeviceIDStore: @unchecked Sendable {
    ...
    /// freshly generated ID so the host always has a usable identity.
