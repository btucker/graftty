...
public final class ZmxLauncher: Sendable {
    ...
    /// shell's* scope instead of the one Graftty asked for. Stripped
    /// once at `GrafttyApp.init()` so every spawn mechanism — direct
    ...
    ///   `exec zmx attach 'graftty-<new-hex>' <shell>` hijacks to the
    ...
    /// at `GrafttyApp.init()`, before any surface spawns. Modifies
    ...
    /// ZMX_SESSION (from a parent Graftty shell, or the developer's
