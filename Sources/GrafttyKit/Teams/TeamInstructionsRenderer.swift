...
public enum TeamInstructionsRenderer {
    ...
    - Add `--remote '<Mac-name-or-device-ID>'` to create on a connected Mac in either direction. The default matches the caller's Git origin, even when project names differ; equivalent GitHub/GitLab SSH and HTTPS URLs match. Use `--project '<destination-name-or-absolute-path>'` to override it. Both Macs must support remote creation and the destination must track the project. Branches and `--base HEAD` resolve on the destination's main checkout; local commits and edits are not transferred.
    - For a remote child, use the returned `graftty-mac://...` address unchanged. Send a follow-up team message so the child can use `graftty team reply` to reach you across Macs. Do not give it a local filesystem path as your remote reply address. After an uncertain creation result, inspect the destination before creating another worktree.
