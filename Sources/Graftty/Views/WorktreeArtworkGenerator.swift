...
enum WorktreeArtworkGenerator {
    ...
    static func generate(name: String, userContext: String, style: WorktreeArtworkStyle,
                         variation: UInt64, theme: WorktreeArtworkTheme?) async throws -> Data {
        try await generate(preferred: {
            let prompt = codexPrompt(name: name, userContext: userContext, style: style, variation: variation, theme: theme)
            ...
            return try await ImageCreatorWorktreeIcon.generate(name: name, userContext: userContext,
                style: style, variation: variation, theme: theme)
        })
    }
    ...
    static func codexPrompt(name: String, userContext: String, style: WorktreeArtworkStyle,
                            variation: UInt64, theme: WorktreeArtworkTheme?) -> String {
        let identity = WorktreeArtworkIdentity(name: name, variation: variation, theme: theme)
        ...
        Subject direction: \(direction.subjectDirection).
        ...
        \(theme?.codexColorInstruction(index: direction.paletteIndex, variation: variation) ?? "Dominant subject palette: \(identity.palette). Use a simple supporting backdrop and a clearly lit subject.")
