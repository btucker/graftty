...
8. **NEW** `Tests/GrafttyMobileKitTests/Remote/SSH/SSHTerminalLoopbackTests.swift` — extends R3's `runAuthLoopback` pattern. Real `RTCPeerConnection` pair + real SSH stack; the `streamFactory` returns a fake echoing `TerminalByteStream` rather than spawning `zmx attach` (iOS Simulator doesn't have host binaries). Exercises the SSH wire end-to-end: env→pty→shell, bytes round-trip both directions, `window-change` forwarding, channel-close kills the stream. `.timeLimit(.minutes(3))` per R3 precedent. iOS-only (UIKit-guarded). Real `zmx attach` integration is verified at the manual TestFlight gate, not in CI.
