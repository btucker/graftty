...
| **R4** | `TerminalSessionHandler` (server) + `TerminalSessionClient` (mobile). `RemoteHostConnection` wired to SSH for terminal — this is the first production protocol over the DataChannel (`ChannelRouter` was never wired into production; only loopback tests exercised it). Terminal I/O round-trip test. iPhone still on `/ws`. | Two terminal paths coexist briefly (`/ws` + SSH session channel) | Medium — `RemoteHostConnection` API change |
