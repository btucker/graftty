...
Phase 1 of bringing [zmx](https://zmx.sh) ([source](https://github.com/neurosnap/zmx)) into Graftty. Every Graftty pane's PTY child becomes `zmx attach <session>` instead of `$SHELL`, so terminal sessions survive Graftty quits, crashes, and relaunches. The native UX is otherwise unchanged.

## Multi-Phase Context

This spec covers Phase 1 only. The full integration arc has three phases:

- **Phase 1 (this spec):** zmx as the PTY backing for every Graftty pane. Survival of running shells across app quits. Native UI unchanged.
...
> I open Graftty, switch to a worktree, and start a long-running build (`cargo build`, `npm run dev`, etc.). I quit Graftty (Cmd-Q). The build keeps running. I relaunch Graftty; my terminal panes restore with the build's full output visible in scrollback, the build still running.

The same survival applies to a crash of Graftty (kill -9, OS forced exit). It does **not** apply to OS reboot — zmx daemons exit on shutdown unless wrapped by `launchd`, which is deferred to a later phase.

## Architecture

One `zmx` daemon per Graftty pane. The pane's libghostty surface owns a PTY whose child is `zmx attach graftty-<short-pane-id>`. zmx in turn spawns `$SHELL` as the daemon's child and proxies PTY bytes between the shell and the attached client (Graftty's surface). Pane lifetime is intentionally decoupled from daemon lifetime — that decoupling is the whole point.

```
Graftty process                         zmx daemon process(es)              shell process(es)
┌───────────────────────────┐            ┌───────────────────────┐          ┌──────────────┐
│ libghostty Surface        │            │ zmx daemon            │          │ $SHELL       │
│   ↕ PTY                   │◄──socket──►│   - libghostty-vt     │◄──PTY───►│              │
│ zmx attach <session>      │            │   - PTY <-> client mux│          │              │
└───────────────────────────┘            └───────────────────────┘          └──────────────┘
       (lives with pane)                  (survives Graftty quit)        (survives Graftty quit)
```

When Graftty quits, the leftmost box dies; the middle and right boxes survive. On relaunch, Graftty creates a fresh leftmost box and `zmx attach` reconnects to the same daemon, which replays the screen state to libghostty's renderer.

### Bundling

The `zmx` binary is bundled inside the app at `Graftty.app/Contents/Helpers/zmx`, mirroring the existing `graftty` CLI placement (per `ATTN-1.1`). MIT license permits bundling. The vendored binary is universal (arm64 + x86_64 via `lipo`) and re-signed with Graftty's developer ID at app-build time.

### Session-name scheme

`graftty-<pane-uuid-short>`, where `<pane-uuid-short>` is the first 8 hex characters of the pane's UUID. Pane UUIDs are already stable across launches (persisted in `state.json` per the `PERSIST` specs), so deriving the session name from the pane id makes reattach automatic — no separate session-name field needs storage. 8 hex chars = 32 bits, ample uniqueness within a single user's `ZMX_DIR` namespace.

### Sandboxing zmx state

Graftty sets `ZMX_DIR=~/Library/Application Support/Graftty/zmx/` in the env it hands to every spawned `zmx attach`. This scopes Graftty's daemons to a private socket directory. A user's personal `zmx list` in Terminal.app uses the default `XDG_RUNTIME_DIR` / `TMPDIR` and won't see Graftty sessions, and vice versa. Clean separation, no accidental collisions.
...
### Modified — `Sources/Graftty/Terminal/`
...
### New — `Sources/GrafttyKit/Zmx/`

- **`ZmxLauncher.swift`** — pure-Swift, no UI deps. Owns:
  - Bundled-binary path resolution (`Bundle.main.bundleURL.appendingPathComponent("Contents/Helpers/zmx")`, mirroring the existing `graftty` CLI resolver at `GrafttyApp.swift:643`; dev-mode fallback to `PATH`).
  - `sessionName(for paneID: UUID) -> String` — deterministic `"graftty-" + paneID.short()`.
  - `attachArgv(sessionName:env:) -> (argv: [String], env: [String:String])` — returns `[zmxPath, "attach", sessionName, "$SHELL"]` plus env including `ZMX_DIR`, `GRAFTTY_SOCK`, `GHOSTTY_RESOURCES_DIR`, and the user's existing env. Working directory is *not* a parameter: libghostty sets the PTY child's cwd via its own spawn API, and `zmx attach` inherits it.
...
- **`Package.swift`** — add a Resource entry to copy `Resources/zmx-binary/zmx` into the Graftty target as an auxiliary executable.
...
- **App bundle install** — extend the existing post-build step that installs `graftty-cli` as `Contents/Helpers/graftty` to also install `Contents/Helpers/zmx`. Re-codesign as part of the same step.
...
4. `ZmxLauncher` returns `[<bundledZmxPath>, "attach", "graftty-<short>", "$SHELL"]` + env including `ZMX_DIR`, `GRAFTTY_SOCK`, `GHOSTTY_RESOURCES_DIR`.
...
### Flow 2 — Graftty launches with existing `state.json`
...
4. `zmx attach` either reattaches (daemon survived Graftty's quit → screen state restored from libghostty-vt buffer) or creates fresh (daemon didn't survive, e.g., reboot → blank shell starts).
...
The principle throughout: **Graftty must remain usable even if zmx is broken.**

### At app launch / first pane creation

- **Bundled `zmx` binary missing or not executable** — `ZmxLauncher.isAvailable == false`. All future `attachArgv` calls return the legacy `$SHELL` argv. Show a one-time, dismissible alert: "zmx unavailable — terminals won't survive quit." Log the cause (file missing? wrong arch? gatekeeper rejection?). Graftty behaves exactly like today's main branch.
- **`ZMX_DIR` not writable** — try to create `~/Library/Application Support/Graftty/zmx/` at first launch. On failure, log and fall back to letting zmx use `TMPDIR`. Graftty's sessions then live alongside any user-private zmx sessions; not ideal but not broken.
- **Codesigning / notarization failure** — bundled zmx must be re-signed with Graftty's developer ID and included in the notarization manifest. The release build script verifies with `codesign -dv` before producing the .app. A CI test asserts the bundled binary is loadable. Without this, macOS Gatekeeper silently kills the spawn and Flow 1 step 6 fails with no obvious cause.
...
- Two Graftty instances sharing one `ZMX_DIR` — works correctly (it's actually the multi-client win) but in dev causes confusion. Document; don't fix.
- Architecture mismatch — the universal binary handles arm64+x86_64. CI verifies via `lipo -info`.

## Testing

### Unit tests — `ZmxLauncherTests` (`Tests/GrafttyKitTests/`)
...
1. Open Graftty; create a worktree; type `echo hello`; quit Graftty; relaunch — `hello` is still visible in the scrollback.
2. With Graftty closed, `ZMX_DIR=~/Library/Application\ Support/Graftty/zmx/ zmx list` shows your panes.
...
### zmx and Graftty both use libghostty

zmx 0.5.0 statically links the `ghostty-vt` Zig module from the Ghostty repo (pinned in `build.zig.zon`). Graftty links the full libghostty (renderer included) via the `libghostty-spm` Swift package. So the integration is two libghostty builds in our process tree doing complementary jobs: Graftty renders the live stream; zmx replays the buffered state to a new client on reattach. This is the intended use of the `ghostty-vt` factoring — Mitchell explicitly split it out so projects like zmx could embed it.
...
Once every pane is zmx-backed, Phase 2's WebSocket server can offer terminal streams to a web client by speaking to the same daemons (via `zmx attach` from a child process, or by reading raw daemon sockets if we ever decide to). Phase 3's TanStack web client gets multi-client sessions for free — two Graftty-shaped clients (native + browser) can attach to the same daemon simultaneously, which is exactly the "open the same shell from my couch" experience.

## Acceptance Criteria

Phase 1 ships when all of the following hold:

1. The `ZmxSurvivalIntegrationTests.sessionSurvivesClientDetachAndReattachRestoresMarker` integration test passes (replaces the spec's original `TerminalManagerSurvivalTests` name; landed in `Tests/GrafttyKitTests/Zmx/` because the Graftty app target has no test target). The end-to-end claim — quit Graftty, relaunch, scrollback marker is restored — is verified by manual smoke test #1 instead.
...
7. With the bundled `zmx` removed (debug build), Graftty launches, creates panes via direct `$SHELL`, surfaces the warning banner, and remains fully usable.
