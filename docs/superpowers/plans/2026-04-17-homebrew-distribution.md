...
**Goal:** Ship Graftty as a Homebrew Cask installable from a personal tap, with releases built and published automatically when a `v*` git tag is pushed.
...
**Modified files:**
- `scripts/bundle.sh` — read `GRAFTTY_VERSION` env var, substitute into `Info.plist`, ad-hoc codesign every Mach-O inner-out
...
- `docs/release/Casks/graftty.rb` — reference cask file content the user commits once into the tap repo
...
The current `bundle.sh` writes a heredoc with quoted delimiter (`<<'PLIST'`) that prevents shell expansion, hardcoding `0.1.0`. We change the delimiter to unquoted (`<<PLIST`) so `$GRAFTTY_VERSION` interpolates, and we read that variable from the environment with a `0.0.0-dev` default for local builds.
...
```bash
cd /Users/btucker/projects/graftty/.worktrees/homebrew-setup
scripts/bundle.sh
/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" .build/Graftty.app/Contents/Info.plist
```
...
/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" .build/Graftty.app/Contents/Info.plist
/usr/libexec/PlistBuddy -c "Print :CFBundleVersion" .build/Graftty.app/Contents/Info.plist
```
...
git commit -m "build: make bundle.sh read GRAFTTY_VERSION from env

Switches the Info.plist heredoc from quoted ('PLIST') to unquoted
(PLIST) so shell variable expansion works, then sources
CFBundleShortVersionString and CFBundleVersion from \$GRAFTTY_VERSION,
...
```bash
scripts/bundle.sh
codesign --verify --strict --verbose=2 .build/Graftty.app
```

Expected output (last two lines):

```
.build/Graftty.app: valid on disk
.build/Graftty.app: satisfies its Designated Requirement
```
...
```bash
codesign --display --verbose=2 .build/Graftty.app/Contents/Helpers/zmx 2>&1 | grep -E "Signature|Identifier"
codesign --display --verbose=2 .build/Graftty.app/Contents/Helpers/graftty 2>&1 | grep -E "Signature|Identifier"
```
...
Signs every Mach-O in the bundle inner-out: zmx, the graftty CLI
helper, the main Graftty binary, then the bundle itself. Ad-hoc
    ...
    steps:
      ...
      - name: Build app bundle
        env:
          ...
          GRAFTTY_VERSION: ${{ steps.version.outputs.version }}
...
scripts/bundle.sh (which now reads GRAFTTY_VERSION from env),
...
a version+sha256 bump to the btucker/homebrew-graftty tap using
...
The cask file lives in the *tap* repo (`btucker/homebrew-graftty`), not here — the workflow updates it via `sed` so it has to exist with the literal `version "..."` and `sha256 "..."` lines for `sed` to match. We ship the initial content as a reference under `docs/release/Casks/graftty.rb` plus a README explaining the one-time setup.

**Files:**
- Create: `docs/release/Casks/graftty.rb`
- Create: `docs/release/README.md`

- [ ] **Step 1: Create the reference cask file**

Write `docs/release/Casks/graftty.rb`:
...
```bash
git add docs/release/Casks/graftty.rb docs/release/README.md
git commit -m "docs(release): initial cask reference + tap setup README

Provides the literal cask file the user copies into the
btucker/homebrew-graftty tap repo on first-time setup, plus a
...
Per `CLAUDE.md`, every PR that adds user-visible functionality must update `SPECS.md`. Distribution behavior is user-visible — this is how users install and update Graftty.
...
**DIST-1.4** The build script shall ad-hoc codesign every Mach-O in the bundle in inner-to-outer order: `Contents/Helpers/zmx`, `Contents/Helpers/graftty`, `Contents/MacOS/Graftty`, then the bundle itself.

### 14.2 Release Automation

**DIST-2.1** When a git tag matching `v*` is pushed to origin, the GitHub Actions workflow `.github/workflows/release.yml` shall build the app bundle in release configuration, verify codesigning, zip the bundle as `Graftty-<version>.zip`, attach the zip to a new GitHub release tagged `v<version>`, and push an updated cask file to the `btucker/homebrew-graftty` tap with the new version and sha256.
...
DIST-1.x: bundle.sh produces a versioned, ad-hoc-signed Graftty.app.
...
brew install --cask graftty
graftty --help
open /Applications/Graftty.app    # right-click first launch for Gatekeeper
