...
4. **Run `scripts/swiftpm test`** to confirm it passes (and that no other tests regressed).
5. **Run `scripts/generate-specs.py`** and commit the updated `SPECS.md` alongside your code.

## Shared SwiftPM builds for local agents

Local agents shall run SwiftPM builds through `scripts/swiftpm`:

```bash
scripts/swiftpm build
scripts/swiftpm test
scripts/swiftpm run graftty-cli -- --help
```
...
- Do not pass `--skip-build`; artifacts in the shared directory may currently
  belong to another worktree. The wrapper rejects it.
- Waiting for another `scripts/swiftpm` command is expected. The global lock
  deliberately favors correctness and bounded disk use over concurrent builds.
- Set `GRAFTTY_SWIFTPM_SHARED_DIR` to an absolute path to override the cache
  location for an isolated experiment.
- CI, release automation, Xcode, and `scripts/bundle.sh` intentionally continue
  to use their own build directories. The wrapper is for local agent CLI
  builds and tests.
- Existing worktree-local `.build` directories are not migrated or removed
  automatically. Remove them only after confirming no local build is running.
