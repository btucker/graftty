...
struct AgentHookInstallerWrapperTests {
    ...
    @Test("""
    @spec TEAM-PRESENCE-1.3: When the graftty wrapper launches an agent runtime with `GRAFTTY_DISABLE_AGENT_HOOKS != 1`, the wrapper shall asynchronously invoke `graftty team register --runtime <runtime>` (output redirected, run in background) before exec'ing the real binary, so the model is not required to type the registration command itself. Pairs with the existing `trap cleanup EXIT` that runs `graftty team unregister` on wrapper exit. The `team register` CLI silently no-ops when the cwd isn't a team-tracked worktree, so the unconditional call is safe.
    """)
    func wrapperRegistersAsynchronouslyBeforeExec() {
        for runtime in [TeamHookRuntime.claude, .codex] {
            let script = AgentHookInstaller.wrapperScript(
                runtime: runtime,
                wrapperDirectory: "/Users/x/agent-hooks/bin",
            ...
            // Register fires before any exec of the real binary so it
            // races with the agent's startup rather than serializing.
