...
""", .disabled("not yet implemented"))
    ...
    @Test("""
@spec WEB-3.5: WebSocket text frames shall carry JSON control envelopes. Legacy `{"type":"resize","cols":<uint16>,"rows":<uint16>}` frames may update the PTY only for a socket that is already the confirmed display owner; ownership-aware clients shall use `hello`, `ownership`, `grid`, `takeControl`, and `ownerResize` envelopes from `WebControlEnvelope`, and only an explicit `takeControl` frame may change web display ownership.
""", .disabled("not yet implemented"))
