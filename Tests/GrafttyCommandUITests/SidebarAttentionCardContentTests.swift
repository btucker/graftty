...
struct SidebarAttentionCardContentTests {
    @Test("@spec LAYOUT-2.73: When a stopped agent has a recap, the Attention card shall show the worktree beside a project icon, a gray pane title beneath it, and Context, Needs You, Up Next in that order; if no project icon exists it shall retain the project name, and if no question exists it shall omit Needs You.")
    func stoppedCardUsesChosenHierarchy() {
        ...
        let withIcon = SidebarAttentionCardContent(item: item, hasProjectIcon: true)
        ...
        #expect(SidebarAttentionCardContent(item: item, hasProjectIcon: false).headerName
            == "graftty · push-notifications")
        ...
        let fallback = SidebarAttentionCardContent(item: noQuestion, hasProjectIcon: true)
