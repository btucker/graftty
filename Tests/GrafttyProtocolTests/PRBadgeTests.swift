...
struct PRBadgeTests {
    ...
    @Test("@spec PR-3.2: While a worktree has a resolved PR/MR, the application shall display a forge-specific reference badge between the leading emoji or fallback icon and worktree label: `#<number>` for GitHub and `!<number>` for GitLab, with ungrouped decimal digits regardless of locale.")
