...
struct ZmxPIDLookupTests {
    ...
    // ~/Library/Application Support/Graftty/zmx/logs/<session>.log.
    ...
    @Test func returnsPIDFromSingleSpawnLine() throws {
        let log = """
        [1776448410787] [info] (default): pty spawned session=graftty-13dafaa9 pid=79730
        [1776448410787] [info] (default): daemon started session=graftty-13dafaa9 pty_fd=6
        """
        let pid = ZmxPIDLookup.shellPID(fromLogContents: log, sessionName: "graftty-13dafaa9")
        #expect(pid == 79730)
    }
    ...
    @Test func returnsLatestPIDWhenMultipleSpawnLines() throws {
        ...
        // a dead PID.
        let log = """
        [1000] [info] (default): pty spawned session=graftty-13dafaa9 pid=11111
        [2000] [info] (default): daemon exited session=graftty-13dafaa9
        [3000] [info] (default): pty spawned session=graftty-13dafaa9 pid=22222
        """
        let pid = ZmxPIDLookup.shellPID(fromLogContents: log, sessionName: "graftty-13dafaa9")
        #expect(pid == 22222)
    }
    ...
    @Test func ignoresSpawnLinesForOtherSessions() throws {
        ...
        // matching.
        let log = """
        [1000] [info] (default): pty spawned session=graftty-other pid=99999
        [2000] [info] (default): pty spawned session=graftty-13dafaa9 pid=22222
        """
        let pid = ZmxPIDLookup.shellPID(fromLogContents: log, sessionName: "graftty-13dafaa9")
        #expect(pid == 22222)
    }
    ...
    @Test func returnsNilForEmptyLog() throws {
        let pid = ZmxPIDLookup.shellPID(fromLogContents: "", sessionName: "graftty-13dafaa9")
        #expect(pid == nil)
    }
    ...
    @Test func ignoresLinesWithMalformedPIDs() throws {
        ...
        let log = """
        [1000] [info] (default): pty spawned session=graftty-13dafaa9 pid=not-a-number
        """
        let pid = ZmxPIDLookup.shellPID(fromLogContents: log, sessionName: "graftty-13dafaa9")
        #expect(pid == nil)
    }
    ...
    @Test func readsPIDFromLogFileOnDisk() throws {
        ...
        try """
        [123] [info] (default): pty spawned session=graftty-abcdef01 pid=4242
        """.write(to: tmp, atomically: true, encoding: .utf8)

        let pid = ZmxPIDLookup.shellPID(
            logFile: tmp,
            sessionName: "graftty-abcdef01"
        )
        #expect(pid == 4242)
    }
    ...
    @Test func readsRotatedLogWhenCurrentHasNoSpawnLine() throws {
        ...
        let current = dir.appendingPathComponent("graftty-session.log")
        let rotated = dir.appendingPathComponent("graftty-session.log.old")

        try """
        [100] [info] (default): pty spawned session=graftty-session pid=4242
        ...
        let pid = ZmxPIDLookup.shellPID(logFile: current, sessionName: "graftty-session")
        #expect(pid == 4242)
    }
    ...
    @Test func currentLogSpawnLineWinsOverRotated() throws {
        ...
        try """
        [100] [info] (default): pty spawned session=graftty-session pid=1111
        """.write(to: rotated, atomically: true, encoding: .utf8)

        try """
        [300] [info] (default): pty spawned session=graftty-session pid=2222
        """.write(to: current, atomically: true, encoding: .utf8)
