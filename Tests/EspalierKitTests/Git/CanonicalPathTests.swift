...
/// --porcelain` emits and thus what Espalier's `state.json` stores.
///
/// Using Foundation's normalizer in `GitRepoDetector` therefore produced
/// paths that didn't match the tracked-worktree index, and `espalier notify`
...
struct CanonicalPathTests {
    ...
    @Test func handlesNonExistentPath() {
        ...
        let result = CanonicalPath.canonicalize("/tmp/espalier-cycle82-nonexistent-zzzzzzz")
        #expect(result.hasSuffix("/espalier-cycle82-nonexistent-zzzzzzz"))
    }
