...
**IOS-8.5** The v1 iOS app shall not use push notifications for PR status, build completions, or session events other than the agent-attention notifications defined in PUSH-1..6.
...
**PUSH-3.1** The APNs alert envelope shall use `apns-topic: com.quotably.graftty`, `apns-push-type: alert`, `apns-collapse-id: "<worktreePath>:<attentionTimestampISO>"`, and a `userInfo` payload matching `AgentStopNotification.content(...).userInfo`.
