...
**TERM-2.5** When a terminal pane's `SurfaceViewWrapper` is unmounted by SwiftUI (worktree switch hiding the previous worktree's panes per `TERM-2.1`), the application shall call `ghostty_surface_set_occlusion(surface, true)` on the underlying surface so libghostty suspends its render loop while the pane is offscreen. When the wrapper is re-mounted (worktree switch restoring panes per `TERM-2.2`), the application shall call `ghostty_surface_set_occlusion(surface, false)` immediately followed by `ghostty_surface_refresh(surface)` so libghostty performs a full clean repaint of the current state. Pre-fix, libghostty kept queueing dirty-region updates while the surface was hidden, and on re-show it painted only those dirty regions — under TUI apps that re-render on every event (Claude Code, htop, vim with autocompletion), this surfaced as a jumbled composite of partial frames stacked over stale text until the user pressed any key (a key event implicitly marks the whole surface dirty inside libghostty). The seam is `TerminalContentView.leafView` attaching `.onAppear` / `.onDisappear` to the wrapper; both hooks call `TerminalManager.setOccluded(_:for:)`, which encapsulates the refresh-on-unhide policy alongside the underlying `ghostty_surface_*` calls so the view layer doesn't reach into libghostty primitives directly.
